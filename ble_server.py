...
HEARTBEAT_CHAR_UUID = 'd1e2f3a4-5b6c-7d8e-9f0a-1b2c3d4e5f6a'
...
# Heartbeat Tracking 
# ============================================
import time as time_module  # Add at top if not already there

last_heartbeat_time = 0.0
heartbeat_lock = threading.Lock()
heartbeat_watchdog_running = True

def update_heartbeat():
    """Called when heartbeat is received from app"""
    global last_heartbeat_time
    with heartbeat_lock:
        last_heartbeat_time = time_module.time()

def get_seconds_since_heartbeat():
    """Returns seconds since last heartbeat"""
    with heartbeat_lock:
        if last_heartbeat_time == 0:
            return float('inf')  # Never received
        return time_module.time() - last_heartbeat_time

def stop_motors_emergency():
    """Send emergency stop to motor controller via UDP"""
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.sendto(b'S', (UDP_IP, UDP_MOTOR_PORT))  # 'S' for STOP
        sock.close()
        print('[Heartbeat] ⚠️ EMERGENCY STOP sent to motors!')
    except Exception as e:
        print(f'[Heartbeat] Failed to send stop: {e}')

def heartbeat_watchdog_thread():
    """Background thread that monitors heartbeat and stops motors if timeout"""
    global heartbeat_watchdog_running
    
    motor_stopped = False  # Track if we already sent stop command
    
    while heartbeat_watchdog_running:
        time_module.sleep(0.25)  # Check 4 times per second
        
        seconds_since = get_seconds_since_heartbeat()
        
        if seconds_since > HEARTBEAT_TIMEOUT:
            if not motor_stopped:
                print(f'[Heartbeat] 💔 No heartbeat for {seconds_since:.1f}s - STOPPING MOTORS')
                stop_motors_emergency()
                motor_stopped = True
        else:
            if motor_stopped:
                print(f'[Heartbeat] 💚 Heartbeat restored!')
                # ==========Send RESUME command ==========
                try:
                    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                    sock.sendto(b'RESUME', (UDP_IP, UDP_MOTOR_PORT))
                    sock.close()
                    print('[Heartbeat] ✅ Sent RESUME to motor controller')
                except Exception as e:
                    print(f'[Heartbeat] Failed to send resume: {e}')
                # ===================================================
            motor_stopped = False
    
    print('[Heartbeat] Watchdog thread exiting')
...
# ============================================
# Heartbeat Characteristic Implementation
# ============================================
class HeartbeatCharacteristic(Characteristic):
    """
    Heartbeat characteristic - receives periodic pings from app
    If no heartbeat received within timeout, motors are stopped
    """
    def __init__(self, bus, index, service):
        ...
        self.add_descriptor(
            CharacteristicUserDescriptionDescriptor(bus, index, self, 'App Heartbeat'))
        
        print(f'[Char] Created Heartbeat characteristic: {HEARTBEAT_CHAR_UUID}')
    
    def WriteValue(self, value, options):
        """Called when app sends heartbeat"""
        try:
            update_heartbeat()
...
class RoverService(Service):
    ...
    def __init__(self, bus, index):
        ...
        heartbeat_characteristic = HeartbeatCharacteristic(bus, 5, self)
        ...
        self.add_characteristic(heartbeat_characteristic)
...
def main():
    ...
    print('[BLE] Starting heartbeat watchdog thread...')
    heartbeat_thread = threading.Thread(target=heartbeat_watchdog_thread, daemon=True)
    ...
    global heartbeat_watchdog_running
    heartbeat_watchdog_running = False
    # ==============================
